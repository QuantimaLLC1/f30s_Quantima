...
// CODE TO INITIALIZE CLIENT
	...
	if (GLOB.currentUserId != null) {
		...
		GLOB.first30SecondsRef = new Firebase('https://f30s-quantima.firebaseio.com/' + GLOB.currentUserId)
	...
	// so the DOM can load.
		...
		GLOB.first30SecondsRef = new Firebase('https://f30s-quantima.firebaseio.com/placeholder');
	};
...
// CHILD REFERENCES TO THE PRIMARY FIREBASE REFERENCE
	...
	GLOB.newUserIdRequestRef = new Firebase('https://f30s-quantima.firebaseio.com/newUserIdRequests/' );
...
// CODE TO SUPPORT PLUGINS AND AUTHENTICATION
	...
	// This directory will be used by all new clients so proper security restrictions should be imposed on this directory to prevent exploits.
		...
		if (GLOB.currentUserId == null) {
			...
			GLOB.newUserIdResponseRef = new Firebase('https://f30s-quantima.firebaseio.com/' + device.uuid + '/authenticationToken');
			...
			GLOB.newUserServerAlertRef = new Firebase('https://f30s-quantima.firebaseio.com/' + device.uuid + '/alerts');
			...
			GLOB.newUserClientAlertRef = new Firebase('https://f30s-quantima.firebaseio.com/' + device.uuid + '/clientEvents');
...
// SERVER LISTENERS
	...
	function updateListeners( authToken) {
				...
				GLOB.first30SecondsRef = new Firebase('https://f30s-quantima.firebaseio.com/' + authToken );
				// Firebase pageReady reference, used when app is fully initialized
